...
import { createDirectus, rest, authentication, createUser, login, readItems, updateItem, createItem, deleteItem, deleteUser, refresh, passwordRequest, passwordReset } from '@directus/sdk';
import { cookies } from 'next/headers';
import { directusAuth, directusAuthUser, directusPrivate, directusPublic } from './directus.config';
 ...
 */
  ...
  export async function resetPasswordStage (token:string,password:string)  {
    try {
      await directusPublic.request(passwordReset(token, password));
    } catch (error:any) {
        throw new Error(error.message);
    }
  }

  export async function resetPasswordLogin (email:string)  {
    try {
      await directusPublic.request(
        passwordRequest(
          email,
          `${API_URL}/resetear-password`  // URL a la que redirigirá el botón del correo
        )
      );
    } catch (error:any) {
        throw new Error(error.message);
